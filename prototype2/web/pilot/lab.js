...
export class LabMap extends PilotMap {
  constructor(canvas,onChange,onError){
    ...
    canvas.addEventListener('pointerup',e=>{if(!this.clickStart||!this.ready||this.mode!=='final'||!this.pinsVisible)return;this.clickStart=null;const r=canvas.getBoundingClientRect(),p=this.world(e.clientX-r.left,e.clientY-r.top);this.select(hitSpot(this.overlay?.spots||[],p,this.scale)?.id||null);});
    ...
    document.addEventListener('visibilitychange',()=>{this.lastTick=0;if(!document.hidden)this.update();});
  }
  ...
  async setScene(id){
    ...
    const overlay=validOverlay(await response.json(),scene.variants.final.sha256);
    if(token!==this.loadToken){for(const im of images.values())im.close();return;}
  ...
  setMode(mode){if(!this.images.has(mode))return;this.mode=mode;if(mode!=='final'){this.setPlaying(false);this.select(null);}this.update();}
  ...
  draw(){
    ...
    if(this.mode==='final'){
      ...
      ac.globalCompositeOperation='destination-out';for(const id of position.behind)ac.drawImage(this.masks.get(id),position.xy[0]-24,position.xy[1]-54,48,60,0,0,48,60);ac.globalCompositeOperation='source-over';
    ...
    if(this.playing&&!document.hidden&&!this.tickTimer)this.tickTimer=setTimeout(()=>{this.tickTimer=0;this.update();},33);
  }
}
