...
export function labPanel(map,key){
  ...
  if(key==='spot'){
    ...
    const landmark=map.overlay.landmark?.id===s.id?(map.overlay.landmark.mode==='independent'?'<p>배경과 외형을 별도로 AI 생성한 합성 시험입니다. 숨겨진 배경은 AI 추정이며 이동·회전·조명 편집은 제공하지 않습니다.</p><button data-lab-clear>선택 해제</button>':map.overlay.landmark.mode==='highlight_only'?'<p>배경 그림 위 선택 윤곽 시험 · 타워 숨김·이동·조명은 제공하지 않습니다.</p><button data-lab-clear>선택 해제</button>':'<p>독립 타워 객체 · 숨긴 뒤 배경은 AI 추정입니다.</p><div class="actions"><button data-tower-light>조명 켜기/끄기</button><button data-tower-visible>타워 숨김/복원</button></div>'):'';
...
export async function bootLab(map,json,{open,close,render}){
  ...
  if(landmark){map.pinsVisible=false;map.routeVisible=false;}
  const scene=params.get('scene')||(landmark?Object.keys(manifest.scenes)[0]:walk?'namsan':'downtown');await map.loadLab(manifest,base,scene);
  ...
  if(landmark){$('#lab-pins').checked=false;$('#lab-route').checked=false;}
  ...
  box.prepend(living);
  if(walk){
    document.body.classList.add('projection-walk');
    document.title='Pixel City · 정사영 남산 산책';$('.pilot-badge').textContent='WALK LAB';$('#map-stage').setAttribute('aria-label','정사영 남산 산책 시험');
    const extra=document.createElement('div');extra.innerHTML='<div class="actions"><button id="walk-reset">처음으로</button><button id="walk-clear">선택 해제</button></div><label><input id="walk-actor" type="checkbox" checked> 캐릭터 표시</label>';
    box.append(extra);$('#lab-scene').parentElement.hidden=!landmark;$('#tower-controls strong').textContent=landmark?'독립 랜드마크 선택·가림':'남산타워 선택·강조';
    if(landmark)$('#tower-focus').textContent='랜드마크 보기';
    $('#tower-light').hidden=true;$('#tower-visible').hidden=true;
    $('#walk-reset').onclick=()=>map.setPhase(0);$('#walk-clear').onclick=()=>map.select(null);
    $('#walk-actor').onchange=e=>{map.actorVisible=e.target.checked;map.update();};
  }
  ...
  map.onFrame=m=>{const enabled=m.mode==='final';for(const id of ['lab-play','lab-pause','lab-phase','lab-spot','lab-tour','lab-next','lab-pins','lab-route','lab-follow'])$('#'+id).disabled=!enabled;
    if(walk)for(const id of ['walk-reset','walk-clear','walk-actor'])$('#'+id).disabled=!enabled;
  ...
  $('#lab-spot').onchange=e=>{if(e.target.value)map.focusSpot(e.target.value);else map.select(null);};
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
    if(b.hasAttribute('data-lab-clear'))map.select(null);
    if(['highlight_only','independent'].includes(map.overlay.landmark?.mode)&&(b.hasAttribute('data-tower-light')||b.hasAttribute('data-tower-visible')))return;
