...
export function labPanel(map,key){
  ...
  if(key==='spot'){
    ...
    const landmark=map.overlay.landmark?.id===s.id?(map.overlay.landmark.mode==='highlight_only'?'<p>배경 그림 위 선택 윤곽 시험 · 타워 숨김·이동·조명은 제공하지 않습니다.</p><button data-lab-clear>선택 해제</button>':'<p>독립 타워 객체 · 숨긴 뒤 배경은 AI 추정입니다.</p><div class="actions"><button data-tower-light>조명 켜기/끄기</button><button data-tower-visible>타워 숨김/복원</button></div>'):'';
...
export async function bootLab(map,json,{open,close,render}){
  ...
  const scene=params.get('scene')||(walk?'namsan':'downtown');await map.loadLab(manifest,base,scene);
  ...
  if(walk){
    ...
    box.append(extra);$('#lab-scene').parentElement.hidden=true;$('#tower-controls strong').textContent='남산타워 선택·강조';
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
    ...
    if(map.overlay.landmark?.mode==='highlight_only'&&(b.hasAttribute('data-tower-light')||b.hasAttribute('data-tower-visible')))return;
