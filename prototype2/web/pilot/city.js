...
import {setupDiscovery,thumbnail} from './discovery.js';
...
export async function bootCity(map,json,{open,close,render,toast}){
  ...
  map.beta={panel(key){
    if(key==='spot')return (document.body.dataset.cityExperience==='discovery'?thumbnail(state.selected,base,true):'')+revealControls()+selected();
  ...
  document.addEventListener('click',e=>{
    ...
    if(b.dataset.cityPlace){state.selected=data.landmarks.find(l=>l.id===b.dataset.cityPlace)||data.places.find(p=>p.id===b.dataset.cityPlace);if(map.overlay?.reveal.auto)map.select(state.selected?.mapSpotId??null);else if(state.selected?.mapSpotId!=='bosingak')map.reveal?.set(false);map.update();open('spot');}
  ...
  if(map.overlay?.traffic.focuses){
    controls.classList.add('city-controls-integrated');
    for(const [i,f] of map.overlay.traffic.focuses.entries()){
      const b=i===0?$('#city-road'):document.createElement('button');b.textContent=f.label+' 차량';b.dataset.trafficFocus=f.id;
      b.onclick=()=>{map.select(null);map.zoom(1);map.center={x:f.xy[0],y:f.xy[1]};map.update();};if(i)controls.append(b);
    }
  }
  if(map.overlay?.reveal.auto){
    const pin=document.createElement('button');pin.textContent='⌖ 보신각';pin.setAttribute('aria-label','가려진 보신각 선택');pin.id='bosingak-marker';pin.style.cssText='position:absolute;z-index:2;transform:translate(-50%,-100%);font-size:11px;padding:3px 6px';$('#map-stage').append(pin);
    pin.onclick=()=>map.focusSpot('bosingak');
    map.onFrame=()=>{const p=map.overlay.spots.find(p=>p.id==='bosingak').xy,x=map.w/2+(p[0]-map.center.x)*map.scale,y=map.h/2+(p[1]-map.center.y)*map.scale;pin.hidden=map.reveal.active||x<20||y<20||x>map.w-20||y>map.h-20;pin.style.left=x+'px';pin.style.top=y+'px';};
    document.addEventListener('keydown',e=>{if(e.key==='Escape')map.select(null);});
  }
  if(manifest.reviewOnly)$('.pilot-badge').textContent='LOCAL REVIEW';
  ...
  if(document.body.dataset.cityExperience==='discovery')setupDiscovery(map,data,base,state,saved,{open,close,render});
