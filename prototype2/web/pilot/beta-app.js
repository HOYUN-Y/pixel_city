...
const titles={chat:'AI 가이드',book:'서울 픽셀 도감',places:'주변 장소',spot:'명소',landmarks:'지도 명소',feed:'피드',upload:'올리기'};
...
function render(){
  const collectedFocus=document.activeElement?.dataset.collect;
  ...
  if(!active)return;
  const title=active==='landmarks'?(map.beta?.landmarkTitle||titles[active]):titles[active],panel=map.beta?.panel(active)??'<p class="sample-note">시안 예시 · 준비 중인 기능입니다. 실제 게시·업로드는 하지 않습니다.</p>';
  ...
  else{const win=document.createElement('section');win.className='floating-window focused';win.setAttribute('role','dialog');win.setAttribute('aria-label',title);win.style.cssText=`left:${document.body.dataset.cityExperience==='discovery'?96:24}px;top:${top}px;width:440px;height:${Math.min(600,innerHeight-top-24)}px;z-index:12`;win.innerHTML=`<header class="window-header" style="background:#8cc5d8"><span class="window-dot"></span><span class="window-title">${title}</span><small>FEEDBACK BETA</small><button class="window-close" data-beta-close aria-label="패널 닫기">✕</button></header><div class="panel-content">${panel}</div>`;$('#windows').append(win);}
  if(collectedFocus)document.querySelector(`[data-collect="${CSS.escape(collectedFocus)}"]`)?.focus({preventScroll:true});
}
function open(key){if(!titles[key])return;opener=document.activeElement;active=key;$('#sheet').classList.toggle('discovery-compact',document.body.dataset.cityExperience==='discovery'&&key==='spot');render();$('[data-beta-close]')?.focus();}
function close(){active=null;render();if(opener?.isConnected)opener.focus();else $('#map').focus({preventScroll:true});}
...
$('#sheet-handle').onclick=()=>{if(document.body.dataset.cityExperience==='discovery'&&active==='spot'){const compact=$('#sheet').classList.toggle('discovery-compact');$('#sheet-handle').setAttribute('aria-label',compact?'상세 시트 크게 펼치기':'상세 시트 작게 접기');}else active?close():open('book');};
